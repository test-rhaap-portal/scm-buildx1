...
```bash
ansible-builder build --file ee.yml --tag ee:latest --container-runtime podman
```
...
- **ee.yml**: This is your EE's "blueprint." It's the main definition file that ansible-builder will use to construct your image.
- **ee-template.yml**: This is the Ansible self-service automation portal template file that generated this. You can import it and use it as a base to create new templates for your portal.
...
```bash
# This command uses your 'ee.yml' file to build an image
# and tags it as 'ee:latest'
...
```bash
ansible-navigator run playbook.yaml --eei ee:latest --pull-policy missing
```
...
podman tag ee:latest your-pah-hostname/ee:latest

# Login to your Private Automation Hub
podman login your-pah-hostname

# Push the image
podman push your-pah-hostname/ee:latest
```
...
podman tag ee:latest your-internal-registry.com/ee:latest
podman login your-internal-registry.com
podman push your-internal-registry.com/ee:latest
```
...
If you want to reuse this execution environment template for future projects, you can import the generated **ee.yml** file into your self-service automation portal.
...
2. **Import from Git repository**: Enter the Git SCM URL containing your `ee.yml` file, click **Analyze** to validate, review the details, then click **Import**.
